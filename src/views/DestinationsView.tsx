...
export const DestinationsView: React.FC<DestinationsViewProps> = ({
  ...
  useEffect(() => {
    ...
    uncached.forEach(d => {
      api.getDynamicPrice(d.id).then(price => {
        ...
        // ML service unavailable ΓÇö silently skip
      });
    });
  }, [destinations]);
  ...
  const vibes = [
    ...
    { id: 'heritage', label: 'Γ¢⌐∩╕Å Heritage & Forts' },
    { id: 'culinary', label: '≡ƒì¢ Culinary & Food' },
    { id: 'beach', label: '≡ƒÅû∩╕Å Coastal & Coral' },
    { id: 'nature', label: '≡ƒî┐ Nature & Valleys' },
    { id: 'wellness', label: '≡ƒºÿ Wellness & Serenity' },
    { id: 'spiritual', label: '≡ƒ¢ò Spiritual Temples' },
    { id: 'adventure', label: '≡ƒºù Scuba & Trekking' },
    { id: 'shopping', label: '≡ƒº╡ GI Handlooms' }
  ];
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-1.5">
                ...
                {(['all', 'domestic', 'international'] as const).map(type => (
                  ...
                  >
                    {type === 'domestic' ? '≡ƒç«≡ƒç│ India Domestic' : type === 'international' ? 'Γ£ê∩╕Å International' : 'All'}
                  </button>
                ))}
              </div>
                  ...
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                    <div>
                      ...
                      <span className="text-sm font-black text-slate-900">
                        {isIndian ? `Γé╣${(dest.startingPrice * 75).toLocaleString('en-IN')}` : `$${dest.startingPrice}`}
                      </span>
                      {dynamicPrices[dest.id] && (
                        <div className="flex items-center gap-1 mt-0.5">
                          <span className="text-[10px] font-bold text-emerald-700">
                            ≡ƒôê Γé╣{dynamicPrices[dest.id].predictedPrice.toLocaleString('en-IN')}
                          </span>
                          ...
                          {dynamicPrices[dest.id].changeVsBaseline !== 0 && (
                            <span className={`text-[9px] font-bold ${dynamicPrices[dest.id].changeVsBaseline > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                              {dynamicPrices[dest.id].changeVsBaseline > 0 ? 'Γåæ' : 'Γåô'}{Math.abs(dynamicPrices[dest.id].changeVsBaseline)}%
                            </span>
                          )}
                        </div>
                      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
          <div className="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden my-8 max-h-[90vh] flex flex-col animate-in fade-in zoom-in-95">
            ...
            <div className="flex gap-2 px-6 pt-3 pb-2 bg-slate-50 border-b border-slate-200 overflow-x-auto scrollbar-none shrink-0">
              {[
                { id: 'overview', label: '≡ƒôî Overview & POIs' },
                { id: 'culture', label: '≡ƒì¢ What\'s Famous Here?' },
                { id: 'local_economy', label: '≡ƒî┐ Local Economy & Homestays' },
                { id: 'best_time', label: 'ΓÅ░ Best-Time & Budgets' },
                { id: 'packages', label: '≡ƒôª Tour Packages' },
                { id: 'reviews', label: `Γ¡É Reviews (${reviews.length})` }
              ].map(t => (
                <button
                  key={t.id}
                  id={`modal-tab-${t.id}`}
                        ...
                        <div key={attr.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex gap-3">
                          ...
                          <div className="flex-1 min-w-0 text-xs space-y-1">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-amber-600 font-bold">Γÿà {attr.rating}</span>
                            </div>
                            ...
                            <div className="flex items-center gap-3 pt-1 text-[11px] font-semibold text-slate-700">
                              <span>ΓÅ▒∩╕Å {attr.estimatedTime}</span>
                              <span>≡ƒÆ░ {attr.entryFee > 0 ? `$${attr.entryFee}` : 'Free Entry'}</span>
                            </div>
                          </div>
                        </div>
                      ))}
                    ...
                    {packages.filter(p => p.destinationId === activeModalDest.id || p.destinationId === activeModalDest.name.toLowerCase()).map(pkg => (
                      <div key={pkg.id} className="p-4 bg-white rounded-2xl border border-slate-200 shadow-2xs flex flex-col justify-between space-y-3">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                          <span className="text-xs text-slate-600 font-medium">ΓÅ▒∩╕Å {pkg.duration}</span>
                          <button
                            onClick={() => {
                              setActiveModalDest(null);
                              onBookPackage(pkg);
